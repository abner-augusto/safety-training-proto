# Revision Evidence — SVR/JBCS 2026 (frozen at `v1.0.0`)

> **Frozen snapshot.** This file is the manuscript evidence bound to tag `v1.0.0`
> (`c2462de`). Do not update its figures as `main` moves on — regenerate them in
> `metrics/revision-evidence.md` instead, and reproduce these with
> `scripts/reproduce-manuscript-evidence.{ps1,sh}`, which check out `v1.0.0` in a
> throwaway worktree.
>
