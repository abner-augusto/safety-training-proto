...
The EvaluatorDashboard is served only in Play mode: `EvaluatorDashboardBootstrap` logs `[SafetyProto] Evaluator Dashboard servers started. HTTP=http://<ip>:8080` (default httpPort 8080, wsPort 7071, path `/eval`). To drive its UI end-to-end, a **BrowserOS MCP** runs at `http://127.0.0.1:9000/mcp` but is **not** registered as a coding-agent MCP server - drive it with `curl` JSON-RPC (stateless: `initialize` -> `tools/list` -> `tools/call`). Useful tools: `new_page` ({url}), `evaluate_script` ({page, expression}), `get_console_logs` ({page}), `save_screenshot` ({page, path} - writes a Windows path like `C:\tmp\x.png`, read back via `/c/tmp/x.png`).
