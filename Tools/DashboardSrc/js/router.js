    ...
    on('SafetyViolation',      p => { state.violationCount++; renderCluster(); addLog(`${t('logViolation')}: ${p.message}`, 'violation'); });
    on('CriticalSafetyFailure',p => { state.violationCount++; renderCluster(); addLog(`${t('logCritical')}: ${p.reason}`, 'violation'); });
