    ...
    function formatDetails(entry) {
      ...
      switch (entry.eventName) {
        ...
        case 'ScoreChanged': {
          ...
          return `${t('detailDelta')}: ${delta >= 0 ? '+' : ''}${delta} · ${t('detailTotal')}: ${d.totalScore ?? 0}`;
        }
        case 'SafetyViolation':
        ...
        case 'CriticalSafetyFailure':
          return `${d.violationCount ?? 0} ${t('violations').toLowerCase()} ${t('detailIn')} ${d.windowSeconds ?? 0}s`;
        case 'SessionReset':
          return t('manualReset');
        case 'SessionCompleted': {
          const m = Math.floor((d.totalElapsedTime ?? 0) / 60);
          const s = Math.floor((d.totalElapsedTime ?? 0) % 60);
          return `${t('detailTime')}: ${m}:${String(s).padStart(2, '0')} · ${t('detailScore')}: ${d.totalScore ?? 0} · ${t('detailCompleted')}: ${d.tasksCompleted ?? 0}/${d.totalTasks ?? 0}`;
        }
        default:
          return '';
      }
