    ...
    const FRAME_EDGE = 0.92;   /* NDC: dispara pouco ANTES de encostar na borda */
    const FRAME_DWELL_MS = 800;  /* tempo fora do quadro antes de mover a câmera */
    const FRAME_GLIDE_MS = 600;  /* duração do deslize */
    ...
    export class EvaluatorViewport {
      constructor(containerEl) {
        ...
        /* Reenquadramento automático: se o participante sai do campo de visão e
           FICA fora por DWELL ms, a câmera desliza atrás dele. A carência evita
           puxar a câmera de quem só encostou na borda e já voltou. */
      ...
      updateFrame(p) {
        ...
        if (Array.isArray(p.ppe)) {
          ...
          /* EPI que saiu do frame saiu da cena (hideWhenEquipped some ao ser vestido):
             sem esta poda a malha ficaria congelada para sempre no slot. Volta sozinha
             se o item reaparecer — _updateObject recria o que não existe. */
      ...
      clearSession() {
        ...
        /* Um reset no meio do deslize não pode deixar a órbita travada. */
      ...
      /* Só conta o tempo fora do quadro; voltar a entrar zera a carência, de modo
         que quem apenas raspa a borda nunca arrasta a câmera atrás de si. */
      _trackFraming(H) {
        if (this._follow.anim) return;                 /* já reenquadrando */
        /* Frames de pose continuam chegando com a aba oculta, mas o rAF para: armar aqui
           desligaria a órbita sem ninguém para religá-la. Rearma quando a aba voltar. */
      ...
      /* Alvo e câmera andam o MESMO delta: o ângulo de órbita e o zoom do avaliador
         sobrevivem ao deslize — a vista escorrega de lado, não gira em torno da cena. */
      _startFollow(H) {
        ...
        this._controls.enabled = false;   /* input e damping não brigam com o deslize */
      }

      /** @returns {boolean} true enquanto um deslize estiver em curso. */
      _stepFollow() {
        const a = this._follow.anim;
        if (!a) return false;

        /* Tempo medido pelos passos, não pelo relógio: se a aba for ocultada no meio do
           deslize o rAF congela, e um dt cru faria a câmera saltar direto para o fim. */
        const now = performance.now();
      ...
      /* Geometria e material do EPI são compartilhados entre as malhas — tirar da cena
         e soltar o rótulo, nunca dar dispose neles. */
      _removePpe(id) {
      ...
      _createObject(id, type) {
        ...
        /* Rótulo apenas para EPIs — cabeça e mãos já são legíveis pela
           forma/legenda; o rótulo fica apagado até uma mão se aproximar */
      ...
      _animate() {
        ...
        const following = this._stepFollow();        /* antes do update: reposiciona câmera+alvo */
      ...
      _updateLabels() {
        /* Um rótulo só "acende" quando é o EPI solto MAIS PRÓXIMO de uma mão
           (dentro do alcance) — com objetos agrupados, acender todos num raio
           poluiria a cena; EPIs vestidos ficam apagados mesmo com a mão perto */
        const NEAR = 0.35; /* m */
