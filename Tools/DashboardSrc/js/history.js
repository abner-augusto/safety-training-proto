    ...
    export function saveSessionToHistory(report, sessionId, participantId) {
      if (!report) return;
      const reportParticipantId = participantId ?? state.currentParticipantId;
      if ((reportParticipantId ?? '').toUpperCase().startsWith('SIM-')) return;
      sessionHistory.unshift({
        sessionId:     sessionId ?? state.currentSessionId,
        participantId: reportParticipantId,
        savedAt:       Date.now(),
        report,
      });
